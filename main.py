...
def genera_html_calendario():
    oggi = datetime.now()
    anno, mese, giorno_oggi = oggi.year, oggi.month, oggi.day
    ...
    for d in ["Lun", "Mar", "Mer", "Gio", "Ven", "Sab", "Dom"]: html += f"<div>{d}</div>"
    html += "</div>"
    for week in cal:
...
def genera_html_archivio():
    ...
    for nome, codice in DIZIONARIO_CAMPIONATI.items():
        ...
        html += f"<tr><td>🟢</td><td style='font-size:12px;'><b>{pulito}</b></td><td><span class='badge' style='background:#475569;'>{codice}.csv</span></td></tr>"
    html += "</tbody></table>"
    return html
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/":
            ...
            res += "<style>body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #0b0f19; color: #e2e8f0; margin: 0; padding: 20px; } .container { max-width: 1200px; margin: 0 auto; } h1 { color: #38bdf8; border-bottom: 2px solid #1e293b; padding-bottom: 10px; margin-bottom: 25px; display: flex; align-items: center; justify-content: space-between; } .layout-main { display: grid; grid-template-columns: 1fr 360px; gap: 30px; } @media (max-width: 900px) { .layout-main { grid-template-columns: 1fr; } } .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 25px; } .card { background: #111827; border: 1px solid #1e293b; padding: 15px; border-radius: 8px; } .card h3 { margin: 0 0 5px 0; color: #94a3b8; font-size: 12px; text-transform: uppercase; } .card .value { font-size: 20px; font-weight: bold; color: #f8fafc; } .card .highlight { color: #10b981; } h2 { color: #f1f5f9; font-size: 17px; margin-top: 0; margin-bottom: 15px; border-left: 4px solid #38bdf8; padding-left: 10px; text-transform: uppercase; letter-spacing: 0.05em; } table { width: 100%; border-collapse: collapse; background: #111827; border-radius: 8px; overflow: hidden; border: 1px solid #1e293b; margin-bottom: 30px; } th { background: #1e293b; color: #38bdf8; text-align: left; padding: 12px; font-size: 13px; } td { padding: 12px; border-bottom: 1px solid #1e293b; font-size: 13px; color: #cbd5e1; } tr:hover { background: #1f2937; } .badge { background: #0284c7; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; } .badge-live { background: #dc2626; animation: pulse 2s infinite; } .cal-box { background: #111827; border: 1px solid #1e293b; border-radius: 8px; padding: 15px; text-align: center; margin-bottom: 25px; } .cal-title { font-weight: bold; color: #38bdf8; margin-bottom: 15px; font-size: 14px; letter-spacing: 0.05em; } .cal-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 5px; margin-bottom: 5px; } .cal-header-days { font-size: 11px; color: #64748b; font-weight: bold; margin-bottom: 10px; } .cal-day { padding: 8px 0; border-radius: 4px; font-size: 12px; font-family: monospace; font-weight: bold; } .cal-day.empty { background: transparent; } .cal-day.regular-day { background: #1e3a8a; color: #f59e0b; } .cal-day.today { background: #ffffff; color: #10b981; box-shadow: 0 0 10px rgba(255,255,255,0.3); } @keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.5; } 100% { opacity: 1; } }</style></head><body><div class='container'><h1><span>⚡ MILLENIUM TERMINAL</span> <span style='font-size:14px;color:#64748b;'>Sistema Online</span></h1><div class='stats-grid'><div class='card'><h3>Ultimo Update</h3><div class='value'>"+DASHBOARD_DATA['ultimo_aggiornamento']+"</div></div><div class='card'><h3>Live Scansionati</h3><div class='value highlight'>"+str(DASHBOARD_DATA['partite_scansionate'])+"</div></div><div class='card'><h3>Alert Telegram</h3><div class='value' style='color:#38bdf8;'>"+str(DASHBOARD_DATA['alert_inviati_totale'])+"</div></div></div><div class='layout-main'><div class='col-left'><h2>🔥 Monitoraggio Live</h2>"
            if not DASHBOARD_DATA["match_rilevanti"]: res += "<div style='background:#111827;padding:20px;border-radius:8px;border:1px solid #1e293b;color:#64748b;margin-bottom:30px;'>Nessun match attivo con i parametri minimi richiesti.</div>"
            else:
                res += "<table><thead><tr><th>Tempo</th><th>Incontro</th><th>Score</th><th>Consiglio / Archivio CSV</th></tr></thead><tbody>"
                for m in DASHBOARD_DATA["match_rilevanti"]: res += f"<tr><td><span class='badge badge-live'>{m['orario']}</span></td><td><b>{m['partita']}</b><br><span style='font-size:11px;color:#64748b;'>{m.get('campionato','-')}</span></td><td><span style='font-family:monospace;font-weight:bold;color:#f59e0b;'>{m['punteggio']}</span></td><td>{m['analisi']}</td></tr>"
                res += "</tbody></table>"
            res += "<h2>📅 Palinsesto Prossimi Match</h2>"
            if not DASHBOARD_DATA.get("match_futuri"): res += "<div style='background:#111827;padding:20px;border-radius:8px;border:1px solid #1e293b;color:#64748b;'>Palinsesto prematch momentaneamente vuoto.</div>"
            else:
                res += "<table><thead><tr><th>Ora</th><th>Incontro</th><th>Campionato</th><th>Analisi Prematch</th></tr></thead><tbody>"
                for mf in DASHBOARD_DATA["match_futuri"]: res += f"<tr><td><span class='badge'>{mf['data_ora']}</span></td><td><b>{mf['partita']}</b></td><td><span style='color:#94a3b8;font-size:11px;'>{mf['campionato']}</span></td><td>{mf['analisi']}</td></tr>"
                res += "</tbody></table>"
            res += "</div><div class='col-right'><h2>📆 Calendario</h2>"+genera_html_calendario()+"<h2 style='margin-top:20px;'>🗄️ Campionati in Archivio</h2>"+genera_html_archivio()+"</div></div></div><script>setTimeout(function(){ location.reload(); }, 15000);</script></body></html>"
            ...
            for partita in partite:
                ...
                if campeonato_live in DIZIONARIO_CAMPIONATI:
                    ...
                    if tiri_totali > 0 or ap_totali > 0:
                        ...
                        nuovi_match_rilevanti.append({"orario": f"{minuto_corrente}'", "partita": f"{squadra_casa} - {squadra_ospite}", "punteggio": f"{gol_casa} - {gol_ospite}", "campionato": campionato_live, "analisi": analisi_output})
        ...
        if response.status_code == 200:
            ...
            for partita in partite:
                ...
                if campeonato in DIZIONARIO_CAMPIONATI and timestamp_inizio > 0:
                    ...
                    prossimi_match.append({"data_ora": ora_inizio, "partita": f"{squadra_casa} - {squadra_ospite}", "campionato": campionato, "analisi": analizza_e_consiglia(nome_file_csv, squadra_casa, squadra_ospite, is_live=False)})
...
if __name__ == "__main__":
    ...
    # RIGA DI TEST PER VERIFICARE LA CONNESSIONE A TELEGRAM
